// Shared primitives for Aubric AML — evidence-board components.
// Exposes: Eyebrow, Rule, Card, Tag, Stamp, KV, Dot, BackendHook, Crosshair, SigLine, Ticker
...
const Stamp = ({ tone, children, style }) => (
  <span className={`stamp ${tone||""}`} style={style}>{children}</span>
);

const BackendHook = ({ name }) => (
  <span className="backend-hook">{name}</span>
);

// Crosshair-cornered card
const Xhair = ({ children, style, className }) => (
  <div className={`xhair ${className||""}`} style={style}>
    <span className="xh-tr" /><span className="xh-br" />
    {children}
  </div>
);
...
// Signature / handwritten line (a drawn-by-hand feeling element on evidence boards)
  ...
  <div style={{display:"flex", flexDirection:"column", gap:2}}>
    <div style={{
      ...
      <svg width={width} height={18} style={{position:"absolute", left:0, top:0, opacity:0.6}}>
        <path d={`M4 13 q ${width*0.15} -8 ${width*0.25} -2 t ${width*0.2} 1 t ${width*0.18} -3 t ${width*0.2} 2`}
  ...
  return (
    <svg width={width} height={height} style={{display:"block"}}>
      {arr.map((v, i) => {
        ...
        const x = i * (bw+1);
        const y = (height - h) / 2;
        return <rect key={i} x={x} y={y} width={bw} height={h} fill={tone}/>;
      })}
    </svg>
  );
};

Object.assign(window, { Eyebrow, Rule, Dot, Tag, Stamp, BackendHook, Xhair, Card, KV, SigLine, Ticker, Spark, Wave });
